...
class API extends CI_Controller {
		...
		if($menu):
			...
			if($bahan):
				foreach($bahan as $bhn):
					...
					$this -> Query -> updateData(array('id_bahan'=>$bhn->id_bahan),$data_update,'bahan');
				endforeach;
                $data['status'] = true;
                $data['msg']	= 'Berhasil';
			else:
    ...
    public function updateStatus(){
        ...
        $edtStok = $this -> Query -> updateData(array('id_transaksi_detail'=>$idtransaksidetail,
                                                    'id_transaksi'=>$idtransaksi,'id_menu'=>$idMenu),
            array('status'=>'disajikan'),'transaksi_detail');
    ...
    public function updateStatus2(){
        ...
        $edtStok = $this -> Query -> updateData(array('id_transaksi_detail'=>$idtransaksidetail,'id_transaksi'=>$idtransaksi,'id_menu'=>$idMenu),
            array('status'=>'dimasak'),'transaksi_detail');
    ...
    public function cekDataMeja(){
        $idMeja 	= $this -> input -> post('idMeja');
        $query      = $this -> Query -> getData(array('id_meja'=>$idMeja, 'status_trans'=>'wait'),'transaksi') -> row();
        if($query):
            $data['status']         = true;
            $data['msg']	        = 'Id meja sedang dalam proses pemesanan, silahkan pilih meja yang kosong';
            //$data['totbayar']       = $query;
        else:
            $data['status']     = false;
            $data['msg']	    = 'Gagal';
    ...
    public function cekBahan(){
        $idmenu = $this->input ->post('idmenu');
        $query  = $this -> Query -> getData(array('id_menu'=>$idmenu),'v_bahan_has_bahan') -> row();
        if($query){
            $data['status'] = true;
            $data['msg']    = $query;
        }else{
